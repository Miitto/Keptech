...
namespace kt::vkh {
  ...
  void Renderer::render() {
    ...
    std::vector<Submesh> submeshes;
    {
      ...
      submeshes = updateObjectsBuffer(frustum);
      VK_TRACE("Draw Deferred");
      drawDeferred(cmdBuf[0], submeshes);
    ...
    vkBeginCommandBuffer(cmdBuf[2], &beginInfo);
    {
      ...
      drawLights(cmdBuf[1], cmdBuf[2], frustum, submeshes);
  ...
  std::vector<Submesh> Renderer::updateObjectsBuffer(const kt::maths::Frustum& frustum) {
    KT_PROFILE_FUNCTION
    ...
    submeshes.reserve(view.size_hint());

    for (const auto& [entity, mesh, transform] : view.each()) {
      submeshes.insert(submeshes.end(), mesh.getSubmeshes().begin(), mesh.getSubmeshes().end());
    ...
    if (objectRequiredSize > buf->buffer.size()) {
      ...
      auto res = Buffer::create(m.vkcore.device.logical, m.vkcore.allocator, bufInfo, allocInfo, "Object Buffer");
  ...
  void Renderer::drawDeferred(VkCommandBuffer cmdBuf, const std::vector<Submesh>& submeshes) {
    KT_PROFILE_FUNCTION
    ...
    std::array<Buffer, 2> buffers{*m.buffers.vertexPositions, *m.buffers.vertexAttribs};
    ...
    for (size_t i = 0; i < submeshes.size(); ++i) {
      ...
      VkDeviceSize vertexPositionOffset = submesh.vertexOffset * sizeof(glm::vec3);
      VkDeviceSize vertexAttribOffset = submesh.vertexOffset * sizeof(VertexAttribs);
      VkDeviceSize meshletOffset = submesh.meshletOffset * sizeof(Meshlet);
      VkDeviceSize meshletVertexOffset = submesh.meshletVertexOffset * sizeof(uint32_t);
      VkDeviceSize meshletTriangleOffset = submesh.meshletTriangleOffset * sizeof(uint32_t);

#ifndef NDEBUG
      VkDeviceSize maxObject = objectOffset + sizeof(Renderer::GpuObject);
      VkDeviceSize maxMeshletOffset = meshletOffset + (submesh.meshletCount * sizeof(Meshlet));
      VkDeviceSize maxMeshletVertexOffset = meshletVertexOffset + (submesh.meshletVertexCount * sizeof(uint32_t));
      VkDeviceSize maxMeshletTriangleOffset = meshletTriangleOffset + (submesh.meshletTriangleCount * sizeof(uint32_t));
    ...
    VkImageMemoryBarrier2 barrier{
        ...
        .image = *m.renderTargets.lights.ssaoResult,
        .subresourceRange =
            VkImageSubresourceRange{
  ...
  void Renderer::drawLights(VkCommandBuffer cmdBuf, VkCommandBuffer combineCmdBuf, const kt::maths::Frustum& frustum,
    ...
    KT_PROFILE_FUNCTION {
      ...
      drawPointLightShadowMaps(cmdBuf, lightInfo, submeshes);
  ...
  std::vector<Renderer::LightRenderInfo> Renderer::updatePointLightsBuffer(const kt::maths::Frustum& frustum) {
    ...
    for (auto [entity, transform, pointLight] : shadowView.each()) {
      ...
      if (pointLight.shadowMap.getIndex() == ~0u) {
        ...
        auto shadowMapRes = Image::create(m.vkcore.allocator, m.vkcore.device.logical, shadowMapCreateInfo, shadowMapAllocInfo,
                                          shadowMapViewCreateInfo, "PointLightShadowMap", false);
      ...
      shadowMaps.emplace_back(pointLight.shadowMap, drawCount);
    }
    ...
    if (lightDataSize > lightBuf->buffer.size()) {
      ...
      auto res = Buffer::create(m.vkcore.device.logical, m.vkcore.allocator, bufInfo, allocInfo, "Point Light Buffer");
    ...
    if (shadowMatrixDataSize > shadowBuf->buffer.size()) {
      ...
      auto res = Buffer::create(m.vkcore.device.logical, m.vkcore.allocator, bufInfo, allocInfo, "Shadow Matrix Buffer");
  ...
  void Renderer::drawPointLights(VkCommandBuffer cmdBuf) {
    KT_PROFILE_FUNCTION;
    seperatedLightsBeginRendering(cmdBuf); // Need to clear regardless
    if (fBufs().pointLights->count == 0) {
      vkCmdEndRendering(cmdBuf);
      return;
    }
  ...
  void Renderer::renderBloom(VkCommandBuffer cmdBuf) {
...

    {
      ...
      for (uint32_t i = 0; i < constants::BLOOM_MIP_LEVELS; i++) {
  ...
  void Renderer::startFrame() {
    ...
    std::array bufInfo = {
        VkDescriptorBufferInfo{
            .buffer = m.buffers.vertexPositions->buffer,
            .offset = 0,
            .range = m.buffers.vertexPositions->occupied(),
        },
        VkDescriptorBufferInfo{
            .buffer = m.buffers.vertexAttribs->buffer,
            .offset = 0,
            .range = m.buffers.vertexAttribs->occupied(),
        },
        VkDescriptorBufferInfo{
            .buffer = m.buffers.indices->buffer,
            .offset = 0,
            .range = m.buffers.indices->occupied(),
        },
        VkDescriptorBufferInfo{
            .buffer = m.buffers.meshlets->buffer,
            .offset = 0,
            .range = m.buffers.meshlets->occupied(),
        },
        VkDescriptorBufferInfo{
            .buffer = m.buffers.meshletVertices->buffer,
            .offset = 0,
            .range = m.buffers.meshletVertices->occupied(),
        },
        VkDescriptorBufferInfo{
            .buffer = m.buffers.meshletTriangles->buffer,
            .offset = 0,
            .range = m.buffers.meshletTriangles->occupied(),
        },
    };

    VkWriteDescriptorSet write{
        .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
        .dstSet = m.globalDescriptorSets.sets[m.frameInfo.index],
        .dstBinding = 2,
        .dstArrayElement = 0,
        .descriptorCount = bufInfo.size(),
        .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        .pBufferInfo = bufInfo.data(),
    };

    vkUpdateDescriptorSets(m.vkcore.device, 1, &write, 0, nullptr);
  }
