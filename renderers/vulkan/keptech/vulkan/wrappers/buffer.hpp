...
namespace kt::vkh {
  ...
  public:
    ...
    static kt::Result<Buffer, VkResult, VK_SUCCESS> create(const Device& device, const BufferCreateInfo& info);
    ...
    Buffer(Buffer&& other) noexcept
        : buffer(other.buffer), _size(other._size), device(other.device), alloc(other.alloc), allocInfo(other.allocInfo),
  ...
  private:
    Buffer(VkBuffer buffer, VkDeviceSize size, const Device& device, VmaAllocation alloc, VmaAllocationInfo allocInfo,
           VkDeviceAddress gpuAddress)
        : buffer(buffer), _size(size), device(device), alloc(alloc), allocInfo(allocInfo), gpuAddress(gpuAddress) {}
