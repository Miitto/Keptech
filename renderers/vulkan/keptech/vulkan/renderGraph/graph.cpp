  ...
  void RenderGraph::execute() {
...

    {
      ...
      for (auto& pass : passes) {
        ...
        imageBarriers.reserve(pass.getBarriers().image.size());
        for (auto& barrier : pass.getBarriers().image) {
          VK_TRACE("Pass '{}': Transitioning image at index {} from layout {} to layout {}", pass.getName(), barrier.resourceId,
        ...
        bufferBarriers.reserve(pass.getBarriers().buffer.size());
        for (auto& barrier : pass.getBarriers().buffer) {
