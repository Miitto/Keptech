...
namespace kt::vkh::setup {
  ...
  namespace {
    ...
    std::expected<Pipeline, std::string> createMeshShaderPipeline(const Renderer::VulkanCore& vkcore, const Formats& formats,
                                                                  const VkDescriptorSetLayout globalLayout) {
      VK_DEBUG("Creating deferred pipeline");

      return createPipeline(::shaders::mesh_shader,
                            GraphicsPipelineConfig{
                                .rendering =
                                    {
                                ...
                                .vertexInput = getVertexInputFromShader(::shaders::mesh_shader, {}),
                                .rasterizer =
                                    {
  ...
  std::expected<Renderer::Pipelines, std::string> createPipelines(const Renderer::VulkanCore& vkcore, const Formats& formats,
    ...
    VKH_MAKE(deferred, createMeshShaderPipeline(vkcore, formats, globalLayout), "Failed to create deferred pipeline.");
