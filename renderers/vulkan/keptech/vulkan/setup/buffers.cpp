...
namespace kt::vkh {
  ...
  std::expected<void, std::string> Renderer::initBuffers() {
    VKH_MAKE(camera,
             createBuffer<components::Camera::Uniforms>(m.vkcore, MAX_FRAMES_IN_FLIGHT, "Camera Buffer", VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                        false, true),
             "Failed to create camera uniform buffer.");
    VKH_MAKE(
        addresses,
        createBuffer<BufferPointers>(m.vkcore, MAX_FRAMES_IN_FLIGHT, "Buffer Addresses", VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, false, true),
        "Failed to create buffer addresses buffer.");
    VKH_MAKE(ssaoKernel,
             createBuffer<glm::vec4>(m.vkcore, constants::SSAO_KERNEL_SIZE, "SSAO Kernel Buffer", VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, true),
             "Failed to create SSAO kernel buffer.");
    VKH_MAKE(vertexPosBuffer,
             createBuffer<glm::vec3>(m.vkcore, INITIAL_VERTEX_COUNT, "Vertex Position Buffer", VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, true),
             "Failed to create vertex position buffer.");
    VKH_MAKE(vertexAttribBuffer,
             createBuffer<VertexAttribs>(m.vkcore, INITIAL_VERTEX_COUNT, "Vertex Attrib Buffer", VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, true),
             "Failed to create vertex buffer.");
    VKH_MAKE(indexBuffer, createBuffer<uint32_t>(m.vkcore, INITIAL_INDEX_COUNT, "Index Buffer", VK_BUFFER_USAGE_INDEX_BUFFER_BIT, true),
             "Failed to create index buffer.");
    VKH_MAKE(materialBuffer, createBuffer<GpuMaterial>(m.vkcore, INITIAL_MATERIAL_COUNT, "Material Buffer", 0, true),
             "Failed to create material buffer.");
    VKH_MAKE(meshletBuffer, createBuffer<Meshlet>(m.vkcore, INITIAL_MESHLET_COUNT, "Meshlet Buffer", 0, true),
             "Failed to create meshlet buffer.");
    VKH_MAKE(meshletVertexBuffer, createBuffer<uint32_t>(m.vkcore, INITIAL_MESHLET_VERTEX_COUNT, "Meshlet Vertex Buffer", 0, true),
             "Failed to create meshlet vertex buffer.");
    VKH_MAKE(meshletPrimitiveBuffer, createBuffer<uint8_t>(m.vkcore, INITIAL_MESHLET_PRIMITIVE_COUNT, "Meshlet Primitive Buffer", 0, true),
             "Failed to create meshlet primitive buffer.");

    std::array<PerFrameBuffers, MAX_FRAMES_IN_FLIGHT> perFrameBuffers;

    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
      VKH_MAKE(objectBuffer, createBuffer<GpuObject>(m.vkcore, INITIAL_OBJECT_COUNT, "Object Buffer"), "Failed to create object buffer.");
      VKH_MAKE(pointLightBuffer, createBuffer<GpuPointLight>(m.vkcore, INITIAL_POINT_LIGHT_COUNT, "Point Light Buffer"),
               "Failed to create point light buffer.");
      VKH_MAKE(shadowMatrixBuffer, createBuffer<glm::mat4>(m.vkcore, INITIAL_SHADOW_MATRIX_COUNT, "Shadow Matrix Buffer"),
