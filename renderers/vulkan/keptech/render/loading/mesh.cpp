...
namespace kt::rdr::loading {
  ...
  uploadVertices(const gltf::MeshData& data, SubdivBuffer<glm::vec3>& positionBuffer, SubdivBuffer<VertexAttribs>& attribBuffer) {
    KT_PROFILE_FUNCTION
  ...
  std::expected<MaybeReallocResult<uint32_t>, std::string> uploadIndices(const gltf::MeshData& data, SubdivBuffer<uint32_t>& indexBuffer) {
    KT_PROFILE_FUNCTION
    ...
    VKH_MAKE(old, realloc(indexBuffer, totalIndicesSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, "Mesh index buffer"),
  ...
  std::expected<MaybeReallocResult<MeshletBufferOffsets>, std::string> uploadMeshlets(const gltf::MeshData& data,
