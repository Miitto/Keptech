...
namespace kt::vkh {
  ...
  kt::Result<Shader, VkResult, VK_SUCCESS> Shader::create(const shaders::Shader& shader) {
    ...
    auto res = vkCreateShaderModule(Renderer::get().getDevice(), &shaderModuleCreateInfo, nullptr, &shaderModule);
    if (res != VK_SUCCESS)
      return {res};
  ...
  void Shader::destroy() {
    if (module != nullptr) {
      vkDestroyShaderModule(Renderer::get().getDevice(), module, nullptr);
      module = nullptr;
    }
  }
} // namespace kt::vkh