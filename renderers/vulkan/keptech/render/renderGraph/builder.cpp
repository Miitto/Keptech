...
namespace kt::rdr {
  ...
  RenderGraph RenderGraphBuilder::build(Renderer& renderer) {
    ...
    for (const auto& [idx, pass] : res.passes | std::views::enumerate) {
      pass.setup(renderer, res.passDescriptors[static_cast<size_t>(idx)].layout);
    }
  ...
  Resources RenderGraphBuilder::buildResources() {
    ...
    for (const auto& [idx, res] : physicalResourceInfos | std::views::enumerate) {
      if (res.isBufferLike()) {
        ...
        auto result =
            Image::create({VK_IMAGE_TYPE_2D, res.format, VkExtent3D{.width = res.size.x, .height = res.size.y, .depth = res.size.z},
