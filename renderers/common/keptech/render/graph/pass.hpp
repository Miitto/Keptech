...
namespace kt::rdr {
  ...
  public:
    ...
    RenderPassBuilder& setGetClearDepthStencilCallback(std::function<bool(DepthClearValue*)> cb);
    [[nodiscard]] std::function<bool(DepthClearValue*)>& getGetClearDepthStencilCallback();
    RenderPassBuilder& setGetClearColorCallback(std::function<bool(unsigned, ColorClearValue*)> cb);
    [[nodiscard]] std::function<bool(unsigned, ColorClearValue*)>& getGetClearColorCallback();

    bool getClearColor(uint32_t attachmentIndex, ColorClearValue* value = nullptr) const;

    bool getClearDepthStencil(DepthClearValue* value = nullptr) const;
    ...
    void setDepthStencilLayout(ImageLayout layout);
    [[nodiscard]] ImageLayout getDepthStencilLayout() const;
  ...
  private:
    ...
    std::function<bool(DepthClearValue*)> getClearDepthStencilCb = nullptr;
    std::function<bool(unsigned, ColorClearValue*)> getClearColorCb = nullptr;
    ...
    ImageLayout depthStencilLayout = KT_IMAGE_LAYOUT_UNDEFINED;

    RenderBufferResource& addGenericBufferInput(const std::string& name
#ifdef KT_VULKAN
                                                ,
                                                VkPipelineStageFlags2 stages, VkAccessFlags2 access, BufferUsageFlags usage
#endif
    );
  };
} // namespace kt::rdr
