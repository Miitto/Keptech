...
#include "keptech/render/graph/pass.hpp"
#include "keptech/render/graph/renderResources.hpp"
...
namespace kt::rdr {
  ...
  public:
    ...
    RenderGraphBuilder& setSwapchainFormat(ImageFormat format) {
      swapchainFormat = format;
      return *this;
    }
    [[nodiscard]] ImageFormat getSwapchainFormat() const { return swapchainFormat; }

  private:
    glm::uvec2 renderResolution{0, 0};
    glm::uvec2 swapchainSize{0, 0};
    ImageFormat swapchainFormat = KT_FORMAT_UNDEFINED;
    ...
    struct Requirement {
      ...
      operator bool() const {
        return layout != KT_IMAGE_LAYOUT_UNDEFINED
#ifdef KT_VULKAN
               || access != 0 || stages != 0
#endif
               || history;
...
#ifdef KT_VULKAN
    VkDescriptorPool buildDescriptorPool(const Renderer& renderer, const Resources& resources);
    std::vector<Descriptors> buildDescriptors(const Renderer& renderer, const Resources& resources, VkDescriptorPool descriptorPool);
#endif
