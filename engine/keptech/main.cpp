...
int main() {
  ...
  bool exitCleanly = false;
  {
    ...
    std::expected<void, std::string> rhiRes = rhi::RHI::init(info.renderer, window);
    if (!rhiRes) {
      KT_CRITICAL("Failed to create RHI: {}", rhiRes.error());
      return -1;
    }
    KT_DEBUG("RHI created successfully");
    ...
    auto setupRes = setupAppLayers(layerStack, window, rgBuilder, rhi);
    if (!setupRes) {
