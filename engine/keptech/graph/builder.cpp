...
template <> struct fmt::formatter<kt::QueueHandoff> : fmt::formatter<std::string_view> {
  template <typename FormatContext> auto format(const kt::QueueHandoff& handoff, FormatContext& ctx) const {
    switch (handoff) {
    case kt::QueueHandoff::No:
      return fmt::formatter<std::string_view>::format("No", ctx);
    case kt::QueueHandoff::ToCompute:
      return fmt::formatter<std::string_view>::format("ToCompute", ctx);
    case kt::QueueHandoff::FromCompute:
...
namespace kt {
  ...
  void RenderGraphBuilder::bake() {
    ...
    KT_REQUIRE(swapchainFormat != rhi::ImageFormat::Undefined, "Swapchain format must be set before baking the render graph.");
  ...
  namespace {
    ...
    std::vector<PassGroup> buildPassGroups(const std::vector<RenderPass>& passes) {
      ...
      for (const auto& pass : passes) {
        ...
        if (currentGroup.count > 0 && (getRawQueue(pass.getQueue()) != currentGroup.queue || !pass.getBarriers().post.image.empty() ||
...
#define VERBOSE(...) KT_DEBUG(__VA_ARGS__)
    VERBOSE("  Requirements:");
    for (const auto& [idx, passId] : passStack | std::views::enumerate) {
      const auto& pass = *passes[passId];
      const auto& reqs = passRequirements[idx];

      VERBOSE("    {}: {} ({})", idx, pass.getName(), *passId);
      VERBOSE("      Read Requirements: {}", reqs.invalidate.size());
      for (const auto& req : reqs.invalidate) {
        VERBOSE("        Resource: {}{}", physicalResourceInfos[req.resourceId].name, req.history ? " (History)" : "");
        VERBOSE("          Layout: {}", req.layout);
#ifdef KT_VULKAN
        VERBOSE("          Access: {}", VkAccessFlags2Formatter(req.access));
        VERBOSE("          Stages: {}", VkPipelineStageFlags2Formatter(req.stages));
#endif
      }
      VERBOSE("");

      VERBOSE("      Write Requirements: {}", reqs.flush.size());
    ...
    for (const auto& [idx, passId] : passStack | std::views::enumerate) {
      ...
      VERBOSE("    {}: {} ({})", idx, pass.getName(), *passId);
      VERBOSE("      Pre-pass Image Barriers: {}", barriers.pre.image.size());
      for (const auto& barrier : barriers.pre.image) {
        const auto& res = physicalResourceInfos[barrier.resourceId];
        VERBOSE("        Resource: {}", res.name);
        VERBOSE("          Old Layout: {}", barrier.oldLayout);
        VERBOSE("          New Layout: {}", barrier.newLayout);
#ifdef KT_VULKAN
        VERBOSE("          Src Stages: {}", VkPipelineStageFlags2Formatter(barrier.srcStages));
        VERBOSE("          Src Access: {}", VkAccessFlags2Formatter(barrier.srcAccess));
        VERBOSE("          Dst Stages: {}", VkPipelineStageFlags2Formatter(barrier.dstStages));
        VERBOSE("          Dst Access: {}", VkAccessFlags2Formatter(barrier.dstAccess));
#endif
        VERBOSE("          Handoff: {}", barrier.handoff);
      }
      VERBOSE("");

      VERBOSE("      Pre-pass Buffer Barriers: {}", barriers.pre.buffer.size());
      ...
      VERBOSE("      Post-pass Image Barriers: {}", barriers.post.image.size());
      ...
      VERBOSE("      Post-pass Buffer Barriers: {}", barriers.post.buffer.size());
...
#undef VERBOSE
  ...
  void RenderGraphBuilder::validatePasses() const {
    for (const auto& passPtr : passes) {
      ...
      for (const auto& [idx, output] : pass.getColorOutputs() | std::views::enumerate) {
        ...
        KT_REQUIRE(
            input->getAttachmentInfo().format != rhi::ImageFormat::Undefined,
  ...
  void RenderGraphBuilder::buildPhysicalResources() {
    ...
    auto updateBuffer = [&](PhysResourceId id, Bitflag<QueueType> queues, Bitflag<BufferUsage> usage, bool host) {
  ...
  void RenderGraphBuilder::buildRequirements() {
    ...
    for (auto& passId : passStack) {
...
#endif
        KT_REQUIRE(barrier.layout == rhi::ImageLayout::Undefined,
...
#endif
        KT_REQUIRE(req.layout == rhi::ImageLayout::Undefined,
      ...
      for (auto& input : pass.getHistoryInputs()) {
        ...
        req.layout = rhi::ImageLayout::ShaderReadOnly;
      }
      ...
      for (auto* input : pass.getStorageImageInputs()) {
        ...
        KT_REQUIRE(
            req.layout == rhi::ImageLayout::Undefined,
        ...
        req.layout = rhi::ImageLayout::General;
      }
      ...
      for (auto* input : pass.getColorInputs()) {
        ...
        if (req.layout == rhi::ImageLayout::ShaderReadOnly) {
          ...
          KT_REQUIRE(req.layout == rhi::ImageLayout::Undefined,
                     ...
                     "pass multiple times.",
                     pass.getName(), input->getName(), req.layout);
          req.layout = rhi::ImageLayout::RenderTarget;
        }
      }
      ...
      for (auto* output : pass.getColorOutputs()) {
        ...
        if (req.layout == rhi::ImageLayout::ShaderReadOnly || req.layout == rhi::ImageLayout::General) {
      ...
      if (dsInput && dsOutput) {
        ...
        if (dstReq.layout == rhi::ImageLayout::ShaderReadOnly) {
          ...
          dstReq.layout = rhi::ImageLayout::General;
          pass.setDepthStencilLayout(rhi::ImageLayout::General);
        } else {
          KT_REQUIRE(dstReq.layout == rhi::ImageLayout::Undefined,
          ...
          dstReq.layout = rhi::ImageLayout::DepthStencilTarget;
          pass.setDepthStencilLayout(rhi::ImageLayout::DepthStencilTarget);
        }
...
#endif
        srcReq.layout = rhi::ImageLayout::DepthStencilTarget;
      } else if (dsInput) {
        auto& dstReq = getInvalidAccess(dsInput->getPhysicalId(), false);
        ...
        if (dstReq.layout == rhi::ImageLayout::ShaderReadOnly) {
          ...
          dstReq.layout = rhi::ImageLayout::DepthStencilReadOnly;
          pass.setDepthStencilLayout(rhi::ImageLayout::DepthStencilReadOnly);
        } else {
          KT_REQUIRE(dstReq.layout == rhi::ImageLayout::Undefined,
        ...
        if (srcReq.layout == rhi::ImageLayout::ShaderReadOnly) {
          KT_DEBUG("Pass '{}': Depth-stencil texture '{}' is used as a shader read-only input and as an output depth stencil attachment."
                   "The layout of the texture for this pass has been set to general. You should probably avoid this, or enable the "
                   "VK_KHR_unified_image_layout extension.",
                   pass.getName(), dsOutput->getName());
          srcReq.layout = rhi::ImageLayout::General;
          pass.setDepthStencilLayout(rhi::ImageLayout::General);
        } else {
          KT_REQUIRE(srcReq.layout == rhi::ImageLayout::Undefined,
                     "Pass '{}': Depth-stencil output '{}' expected to have undefined layout. Has {}. You have probably added this "
                     "resource to this pass multiple times.",
  ...
  namespace {
    struct ResInfo {
      ImageLayout layout{rhi::ImageLayout::Undefined};
#ifdef KT_VULKAN
...
#ifdef KT_VULKAN
              || resInfo.access != req.access || resInfo.stages != req.stages
#endif
          ) {
  ...
  Resources RenderGraphBuilder::buildResources() {
    ...
    for (const auto& [idx, res] : physicalResourceInfos | std::views::enumerate) {
      if (res.isBufferLike()) {
        ...
        auto result = Image::create({ImageDim::e2D, res.format, res.size, res.imageUsage, res.mipLevels, res.layers, res.name.c_str()});
  ...
  ResourceInfo RenderGraphBuilder::getResourceInfo(RenderTextureResource& resource) const {
    ...
    if (resInfo.format == rhi::ImageFormat::Undefined)
