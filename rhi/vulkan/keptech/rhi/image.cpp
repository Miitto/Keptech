...
namespace kt::rhi {
  kt::Result<Image, VkResult, VK_SUCCESS> Image::create(const ImageCreateInfo& info) {
    ...
    auto res = vmaCreateImage(r.vkGetDevice(), &info.getImageInfo(), &info.getAllocInfo(), &image, &alloc, &aInfo);
    if (res != VK_SUCCESS) {
      return {res};
    }
    ...
    Image i(info.getName(), imgType, image, view, alloc, info.getExtent(), info.getFormat(), info.getUsage());
    if (info.getName()) {
      r.vkGetDevice().setAllocationName(i.alloc, info.getName());
      r.vkGetDevice().setDebugName(i, info.getName());
    }
  ...
  void Image::destroy() {
    if (image) {
      vmaDestroyImage(RHI::get().getDevice(), image, alloc);
      vkDestroyImageView(RHI::get().getDevice(), view, nullptr);
      image = nullptr;
      view = nullptr;
      alloc = nullptr;
    }
