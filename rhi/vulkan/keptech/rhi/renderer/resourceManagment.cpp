...
namespace kt::rhi {
  bool RHI::canRenderToFormat(VkFormat format) const {
  ...
  std::expected<gltf::Scene, std::string> RHI::loadMesh(std::string_view path) {
    KT_PROFILE_FUNCTION
  ...
  std::expected<RHI::UploadResult<Image>, std::string> RHI::createImages(const gltf::Data& gltf, const VkCommandBuffer transferCmd) {
    KT_PROFILE_FUNCTION
  ...
  std::expected<RHI::UploadResult<Mesh>, std::string> RHI::uploadMeshes(const std::vector<gltf::MeshData>& meshes,
  ...
  std::expected<RHI::UploadResult<Material>, std::string> RHI::createMaterials(const gltf::Data& data, const std::vector<Image>& textures,
