...
namespace kt::rhi {
  ...
  std::expected<void, std::string> RHI::initPhysicalDevice(const RendererCreateInfo& createInfo) {
    VKH_MAKE(selector, kt::rhi::PhysicalDeviceSelector::create(m.vkcore.instance), "Failed to create physical device selector.");
  ...
  std::expected<void, std::string> RHI::initLogicalDevice(const RendererCreateInfo& createInfo,
  ...
  std::expected<std::set<uint32_t>, std::string> RHI::initDevice(const RendererCreateInfo& createInfo) {
